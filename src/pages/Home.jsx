const socialLinks = [
  ...
  },
  {
    ...
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        <path d="M4.98 3.5C4.98 4.88 3.88 6 2.5 6S0 4.88 0 3.5 1.12 1 2.5 1s2.48 1.12 2.48 2.5zM.2 8.98h4.6V24H.2V8.98zM8.98 8.98h4.42v2.05h.06c.62-1.17 2.14-2.4 4.4-2.4 4.7 0 5.57 3.09 5.57 7.11V24h-4.6v-7.77c0-1.85-.03-4.23-2.58-4.23-2.58 0-2.98 2.02-2.98 4.09V24H8.98V8.98z" />
      </svg>
    ),
  },
  {
...
const heroButtons = [
  { label: "Hire Me!", href: "#contact", outline: true },
  { label: "Download CV", href: "/public/Griffins_Kimotho_CV.txt", outline: false },
];
      ...
      <div className="hero-inner">
        <aside className="side-socials" aria-hidden="false">
          {socialLinks.map((link) => (
            <a key={link.label} href="#" className="social-vertical" aria-label={link.label}>
              {link.icon}
            </a>
          ))}
        </aside>
          ...
          <div className="hero-buttons">
            {heroButtons.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className={`button${item.outline ? " outline" : ""}`}
              >
                {item.label}
              </a>
            ))}
